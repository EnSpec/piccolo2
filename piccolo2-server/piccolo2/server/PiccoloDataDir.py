...
class PiccoloDataDir(object):
    ...
    @property
    def latestFileData(self):
        return self._latestFile
    
    def updateLatestFile(self,fname=None,data=None,path='',pattern='*.pico'):
        if data is not None:
            self._latestFile = data
        elif fname is not None and os.path.exists(self.join(fname)):
            self._latestFile = self.getFileData(fname)
        elif fname is not None:
            self.log.warn("New latest file {} does not exist".format(fname))
        else:
            self._latestFile =  self.getFileData(self.getFileList(path,pattern,-1)[0])
