...
class DouglasPeuckerSimplifier(object):
    ...
    def __init__(self,xs,ys,tolerance):
        self.xs = xs
        self.ys = ys
        #uses squared segment distance to avoid sqrt
...
def simplify(xs,ys, tolerance=0.1, highestQuality=True,f2py=True):
    #smooth it out first
    xs = savitzky_golay(xs,25,5)
    if f2py:
        #use a more performant FORTRAN routine at the expense of readability
        markers = dpsimplify.simplify(xs,ys,tolerance).astype('bool')
        return xs[markers],ys[markers],np.where(markers)[0]
    else:
        return DouglasPeuckerSimplifier(xs,ys,tolerance).simplifyDouglasPeucker()
