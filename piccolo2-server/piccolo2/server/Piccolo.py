...
class PiccoloOutput(threading.Thread):
    ...
    def run(self):
        while True:
            ...
            self._datadir.updateLatestFile(data=
                    spectra.serialize(pretty="False",spectrum="Light"))
            if self._preview:
                self.log.info('Preview spectrum updated to {}'.format(spectra.outName))
            else:
                self.log.info('writing {} to {}'.format(spectra.outName,
                    self._datadir.datadir))
                try:
                    spectra.write(prefix=self._datadir.datadir,
                            clobber=self._clobber,split=self._split)
                except RuntimeError, e:
                    self.log.error('writing {} to {}: {}'.format(spectra.outName,
                        self._datadir.datadir,e))
...
class Piccolo(PiccoloInstrument):
    ...
    def getSpectra(self,fname='',chunk=None,simplify=False,outDir=''):
        ...
        if fname=='':
            using_latest = True
            data = self._datadir.latestFileData
        else:
    ...
    def setPreview(self,status=False):
        """If previewing, record but don't save files."""
        self._output.setPreview(status)
