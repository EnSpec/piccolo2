...
class PiccoloQuerryThread(QtCore.QThread):

    pySignal = QtCore.pyqtSignal(object)    
    ...
    def piccoloConnect(self,piccolo):
        self._piccolo = piccolo
        self.call_func = getattr(self._piccolo.piccolo,self.remote_call)
        self.start()

    #def setCallback(self,callback):
    #    self.pySignal.connect(callback)

    def run(self):
        while True:
            if self._piccolo is None:
                time.sleep(.1)
            else:
                task = self.tasks.get()
                result = self.call_func(**task)
                self.pySignal.emit(result)
...
class PlayerApp(QtGui.QMainWindow, player_ui.Ui_MainWindow):
    def __init__(self, parent=None):
        ...
        # set up Piccolo Querrying threads
        self.spectraDownloader = PiccoloQuerryThread("getSpectra")
    ...
    def floatToUnits(self,val,units):
        ...
        try:
            return '{:.2f} {:s}'.format(float(val),units)
        except Exception:
            return str(val)
    ...
    def telemetryStatus(self):
        ...
        if self.tabWidget.currentIndex()==0 and self._piccolo!=None:
            ...
            if "Altimeter" in self._peripherals:
                ...
                self.altimeterLabel.setText(self.floatToUnits(pAltitude,"m"))
            else:
    ...
    def status(self):
        # handle status
        # don't handle status while spectra are autodisplaying
    ...
    def startRecording(self,start=None,end=None,interval=None):
        ...
        if n==0 or self.infRepeat.isChecked() or self._previewing:
            n='Inf'
        kwds ={}
    ...
    def togglePreview(self):
        ...
        self._piccolo.piccolo.setPreview(status=self._previewing)
    ...
    def downloadLatestSpectrum(self):
        ...
        self.updateSelectors = False
        self.spectraDownloader.call(outDir = odir,fname='',simplify=simplify)
        #self._spectra = self._piccolo.piccolo.getSpectra(outDir=odir,
        #        fname='', simplify = simplify)

        #self._plotDownloadedSpectrum(updateSelectors=False)

    def updateRealTime(self):
        if ((self.showLatest.isChecked() or self._previewing) 
    ...
    def downloadSpectra(self,spectraName=None):
        ...
        self.updateSelectors = True
        self.spectraDownloader.call(fname=spectraName, simplify = self.downloadSimple.isChecked())

    def _plotDownloadedSpectrum(self,spectra):
        ...
        if not self.updateSelectors:
            #remember the previous set of view selectors
            spectrum = self.selectSpectrum.currentText()
            direction = self.selectShutter.currentText()
            spectrometer = self.selectSpectrometer.currentText()
        ...
        if not self.updateSelectors:
            #restore view selectors that still exist
            f=lambda c,v:c.setCurrentIndex((c.findText(v)+1 or 1)-1)
            f(self.selectSpectrum,spectrum)
            f(self.selectShutter,direction)
            f(self.selectShutter,direction)
    ...
    def showSpectra(self):
        ...
        units = self.displayUnits.currentText()
        self.spectraPlot.plotSpectra(spectra,directions,spectrometers,units)
    ...
    def connect(self,connection,data):
        ...
        # hookup query threads
        self.spectraDownloader.piccoloConnect(self._piccolo)
        self.spectraDownloader.pySignal.connect(self._plotDownloadedSpectrum)
